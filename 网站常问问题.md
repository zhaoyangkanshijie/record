...
如何绑定事件?绑定同一元素的执行顺序。绑定层叠元素的执行顺序。冒泡与捕获区别。如何阻止冒泡？介绍事件委托。自定义事件实现。
...
闭包的含义与作用。垃圾回收机制。闭包如何造成内存泄漏？内存泄漏监控与解决方法。
...
vue2与vue3的不同。vue3有什么新特性？diff上的优化
...
v-for为什么需要key属性？循环不用index为key？dom diff原理
