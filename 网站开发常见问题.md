        ...
        * await 强制下面代码等待，直到Promise对象resolve(即使await 其它，也需耗时)，得到resolve的值作为await表达式的运算结果
            ...
            let problem2 = () => {
                ...
                let timer = setInterval(()=>{
                    ...
                    if(bufferCount > 0 && totalCount > 0){
                        let response = getResult(3000);
                        bufferCount--;
                        totalCount--;
                        response.then((data)=>{
                            bufferCount++;
                            result.push(data);
                            console.log(2,bufferCount,totalCount,result,result.length)
                        });
                    }
                    console.log(1,bufferCount,totalCount,result,result.length)
                },500);
            }
            problem2();
            ```
