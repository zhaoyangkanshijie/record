...
2. 详解
  ...
  - Jest使用
    ...
    describe('Should sum function run correctly', () => { 
      ...
      expect (sum(l, 2)) _toBe(3); // toXXX:匹配器 
      // toEqual:判断值是否相等 
      expect(sum(l, 2)).toEqual(3);
      // toBeDefined:判断是否被定义 
      expect(sum(l, 2)).toBeDefined();
      // toBeUndefined:判断是否未被定义
      expect(sum(l, 2)).not.toBeUndefined(); // not.toXXX:取反
      // toBeTruthy:判断是否为真值（true、非零数字、非空字符串、对象/数组等) 
      expect(sum(l, 2)).toBeTruthy();
      ...
      expect(sum(l, 2)).toBeLessThan0rEqual(3);
      });
    });
    ```
    ...
    const compileAndroidCode = 0： Error => { 
    ...
    it('promise reject', () => { 
      expect _assertions(1);//保证1条断言被调用 
